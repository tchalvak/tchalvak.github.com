...
/* /home/john/www/bighugelabs.com/js/mootools-release-1.11.js ******************************************************/
...
/* /home/john/www/bighugelabs.com/js/flickrtoys.js ******************************************************/
...
// Show photo browser
function _openPhotoBrowser(callback, uploads) {
	if (uploads == null) uploads = 0;
	hwnd = window.open('flickr-browser.php?callback='+callback+'&uploads='+uploads, 'browser', 'width=420,height=350,scrollbars=1,location=0,resizable=1,dependent=1,toolbar=0,menubar=0,status=0');
}
...
function _imageChooserTabSelect(tab) {
	for(var i = 0;i <= 10;i++) {
		if ($('imagechooser_'+i) == null) break;

		$('imagechooser_'+i).setStyle('display', 'none');
		$('imagechooser_'+i+'_tab').style.backgroundColor = '#fff';
		$('imagechooser_'+i+'_tab').style.borderBottom = '1px solid #ccc';
		$('imagechooser_'+i+'_tab').style.padding = '2px 5px 5px 5px';
		$('imagechooser_'+i+'_tab').style.color = '#ccc';
	}
	$('imagechooser_99').setStyle('display', 'none');
	$('imagechooser_99_tab').style.backgroundColor = '#fff';
	$('imagechooser_99_tab').style.borderBottom = '1px solid #ccc';
	$('imagechooser_99_tab').style.padding = '2px 5px 5px 5px';
	$('imagechooser_99_tab').style.color = '#ccc';

	$('imagechooser_'+tab).setStyle('display', 'block');
	$('imagechooser_'+tab+'_tab').style.backgroundColor = '#fafafa';
	$('imagechooser_'+tab+'_tab').style.borderBottom = '0';
	$('imagechooser_'+tab+'_tab').style.padding = '6px 5px 6px 5px';
	$('imagechooser_'+tab+'_tab').style.color = '#000';
	$('imagechooser_'+tab+'_tab').blur();

	$('imagechooser_message').setStyle('display', 'none');;
	$('imagechooser_message').innerHTML = '';
...
function _chooseTab(tab) {
	for(var i=1;i<=99;i++) {
		if ($('tab_'+i)==null) break;

		$('tab_'+i).setStyle('display', 'none');
		$('tab_'+i+'_tab').style.backgroundColor = '#fafafa';
		$('tab_'+i+'_tab').style.borderBottom = '1px solid #ccc';
		$('tab_'+i+'_tab').style.padding = '3px 5px 5px 5px';
		$('tab_'+i+'_tab').style.color = '#ccc';
	}

	$('tab_'+tab).setStyle('display', 'block');
	$('tab_'+tab+'_tab').style.backgroundColor = '#fff';
	$('tab_'+tab+'_tab').style.borderBottom = '0';
	$('tab_'+tab+'_tab').style.padding = '6px';
	$('tab_'+tab+'_tab').style.color = '#000';
	$('tab_'+tab+'_tab').blur();
}
...
function _imageChooserMessage(msg) {
	...
	$('imagechooser_message').innerHTML = msg;
	$('imagechooser_message').setStyle('display', 'inline');
}

// Copy url to the hidden url variable
function _imageChooserSelectImage(url) {
	$('imagechooser_url').value = url;
}

// Submit a form
function _submitform(f, action) {
	if ($(f) != null) {
		if ($(f).action != null) {
...
function _fadeout(id, t) {
	...
	new Fx.Style(id, 'opacity', { duration: t, onComplete: function(e){ e.setStyle('display', 'none')} } ).start(1,0);
}
...
function _fadein(id, t) {
	...
	new Fx.Style(id, 'opacity', { duration: t } ).start(0,1);
}

function _openEmailWindow(file) {
	window.open('email.php?file='+escape(file), 'emailwindow', 'width=400,height=510,toolbar=0,menubar=0,scrollbars=1,location=0,status=0');
}
...
function _initPhotoBrowser(kind) {
	...
	new Ajax('/_browser.php',
		{
			data: 'kind='+escape(kind),
			update: $('pb'),
			onSuccess: function() { _updatePhotoBrowserDraggables() }
		}
	).request();

	if (_photo_browser_init == 0) {
		_photo_browser_init = 1;
		$$('#droptargets div.droppable').each(function(drop, index){
			drop.addEvents({
				'over': function(el, obj){
					this.setStyle('background-color', '#ffe');
					this.setStyle('border-style', 'solid');
					this.setStyle('border-color', '#f90');
					el.setStyle('z-index', '100');
				},
				'leave': function(el, obj){
					if (this.innerHTML == '') {
						this.setStyle('background-color', '#fff');
						this.setStyle('border-style', 'dashed');
						this.setStyle('border-color', '#ccc');
					} else {
						this.setStyle('background-color', '#fff');
						this.setStyle('border-style', 'solid');
						this.setStyle('border-color', '#090');
					}
				},
				'drop': function(e, obj){
					var p = e.getParent();
					var clone = e.clone();
					clone.setStyles({'position': 'relative', 'top': '0', 'left': '0', 'opacity': '1', 'z-index': '99'});
					this.setStyle('border-style', 'solid');
					this.setStyle('border-color', '#090');
					this.innerHTML = '';
					clone.inject(this);
					...
					if (p.innerHTML == '') {
						p.setStyle('background-color', '#fff');
						p.setStyle('border-style', 'dashed');
						p.setStyle('border-color', '#ccc');
					}
					...
					clone.makeDraggable({droppables: $$('#droptargets div.droppable')});
		...
		$$('#droptargets div.droppable img').each(function(item) {
			item.addEvent('emptydrop', function() {
				var p = this.getParent();
				this.remove();
				if (p.innerHTML == '') {
					p.setStyle('background-color', '#fff');
			...
			item.makeDraggable({droppables: $$('#droptargets div.droppable')});

			var p = item.getParent();
			p.setStyle('border-style', 'solid');
			p.setStyle('border-color', '#090');
		});
...
function _getPhotosForm(action) {
	...
	if ($('pb_kind')) kind = $('pb_kind').value;
	if (kind == null) kind = 'flickr';

	if ($('pb_set')) set = $('pb_set').value;
	if (set == null) set = '';

	if ($('pb_user')) user = $('pb_user').value;
	if (user == null) user = '';

	if ($('pb_tag')) tag = $('pb_tag').value;
	if (tag == null) tag = '';

	new Ajax('/_browser.php',
		{
			data: 'page='+escape(page)+'&kind='+escape(kind)+'&set='+escape(set)+'&user='+escape(user)+'&tag='+escape(tag),
			update: $('pb'),
			onSuccess: function() { _updatePhotoBrowserDraggables() }
		}
	).request();
...
function _updatePhotoBrowserDraggables() {
	$$('#pb_photos img.draggable').each(function(item){
		item.addEvent('mousedown', function(e) {
			e = new Event(e).stop();
	 
			var clone = this.clone()
				.setStyles(this.getCoordinates())
				.setStyles({'opacity': '0.7', 'position': 'absolute', 'display': 'block'})
				.addEvent('emptydrop', function() {
					this.remove();
				}).inject(document.body);
	 
			var drag = clone.makeDraggable({
				droppables: $$('#droptargets div.droppable')
			});
...
function _getPhotoBrowserPhotos() {
	...
	$$('#droptargets div.droppable').each(function(item) {
...
var _t_loading = setTimeout("_hideLoadingMessage()", 30000);
...
function _showUploadTargets() {
	var p = $("uploadbutton").getCoordinates();

	$("uploadtargets").style.left = (p.left) + "px";
	$("uploadtargets").style.top = (p.top + 25) + "px";
	_show("uploadtargets");

	return false;
}

function _hideUploadTargets() {
	_hide("uploadtargets");
}
